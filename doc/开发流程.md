...
### 20180819 更新
    ...
    - 添加配置文件解析
      ...
      bingo 使用yaml文件来组织配置文件
      
      像laravel一样，config目录下随意创建目录和文件
      
      例如创建文件 config/cc/app.yaml
      
      app.yaml的内容是:
      
        config : example
    
      那么在程序中使用 `bingo.Config.Get()` 方法来获取配置
      
      ```
        c,err := bingo.Config.Get("cc.app.config")
        
        // 返回值为 example和nil （多层目录可以使用 . 号分割获取,yaml文件里也可以使用多级结构，但是暂时不支持节点下是数组结构）
         
        res := bingo.Config.Set("key","value")  // 设置配置项，返回值为true
      ```
